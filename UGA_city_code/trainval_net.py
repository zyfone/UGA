...
if __name__ == '__main__':
	...
	elif args.dataset == "cityscape_gan":
		args.s_imdb_name = "cityscape_2007_train_s+cityscape_gan_2007_train_s"
		args.t_imdb_name = "cityscape_2007_train_t"
		args.t_imdbtest_name = "cityscape_2007_test"
		args.set_cfgs = [
			"ANCHOR_SCALES",
	...
	for epoch in range(args.start_epoch, args.max_epochs + 1):
		...
		for step in range(iters_per_epoch):
			...
			da_ins_loss = args.lamda1 * 0.5 * (
				...
				+
				#new loss
				(Align(inst_feat_pos, domain=0) \
	 			+Align(t_inst_feat_pos, domain=1)\
				+Align(inst_feat_neg, domain=0) \
				+Align(t_inst_feat_neg, domain=1))*0.25
			)
			...
			loss = sv_loss + da_img_loss + da_ins_loss+(loss_channel_s+loss_channel_t)*0.01
			...
			if step % args.disp_interval == 0:
				...
				loss_dict = _reset_dict(loss_dict)
				start = time.time()
				if epoch >= 6 and step%1000==0 and step>0:
					save_name = os.path.join(
						output_dir, 
						f'da_faster_rcnn_{args.dataset}_{epoch}_{step}.pth'
					)
					save_checkpoint(
						{

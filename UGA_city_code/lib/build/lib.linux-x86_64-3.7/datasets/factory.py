...
from datasets.cityscape_gan import cityscape_gan
...
for year in ["2007"]:
    for split in ["train_s", "train_aug", "train_t", "test",'test_t']:
...
for year in ["2007"]:
    for split in ["train_s"]:
        name = "cityscape_gan_{}_{}".format(year, split)
        __sets[name] = lambda split=split, year=year: cityscape_gan(split, year)

...
from model.faster_rcnn.DA import _InstanceDA_channel
...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic, context):
        ...
        self.channel_DA = _InstanceDA_channel(256)
    ...
    def forward(self, 
        ...
        channel_feat=self.channel_DA(ReverseLayerF.apply(base_feat_gcn, 0.1))
        ...
        inst_feat_neg =None
        if self.training:
            inst_feat_pos = self.DA_inst_pos(ReverseLayerF.apply(pooled_feat_pos, alpha1))
            inst_feat_neg = self.DA_inst_neg(ReverseLayerF.apply(pooled_feat_neg, alpha1))

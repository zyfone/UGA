...
class _ImageLayer3(nn.Module):
    ...
    def forward(self, x):
        if self.db:
            ...
            feat_size = x.size()[2]
            keep_rate = max(1.0 - self.drop_rate / (4*10000) * (self.num_batches_tracked),torch.tensor( 1.0 - self.drop_rate))
...
class vgg16(_fasterRCNN):
    def __init__(self, classes, pretrained=False, class_agnostic=False, context=True, db=False, init=False):
        self.model_path = "/root/autodl-tmp/pretrained_model/vgg16_caffe.pth"
    ...
    def _init_modules(self):
        ...
        self.DA_inst_pos = _InstanceLayer(self.dout_base_model, self.db)
        self.DA_inst_neg = _InstanceLayer(self.dout_base_model, self.db)
    ...
    def _head_to_tail(self, pool5):
        self.num_batches_tracked= torch.tensor(4*10000)
        if self.db:
            self.num_batches_tracked += 1

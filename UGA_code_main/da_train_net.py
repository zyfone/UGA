...
from model.da_faster_rcnn_da.resnet import resnet
from model.da_faster_rcnn_da.vgg16 import vgg16
...
if __name__ == "__main__":
    ...
    if args.dataset == "flir":
        ...
        args.s_imdb_name = "flir_vis_2007_trainval"
        args.t_imdb_name = "flir_tr_2007_trainval"
        args.s_imdbtest_name = "flir_tr_2007_test"
        args.t_imdbtest_name = "flir_tr_2007_test"
        args.set_cfgs = [
            "ANCHOR_SCALES",
            "[4,8,16,32]",
            "ANCHOR_RATIOS",
    ...
    elif args.dataset == "flir_city":
        print("loading our dataset...........")
        args.s_imdb_name = "cityscape_thermal_2007_train_s"
        args.t_imdb_name = "flir_tr_2007_trainval"
        args.s_imdbtest_name = "flir_tr_2007_test"
        args.t_imdbtest_name = "flir_tr_2007_test"
        args.set_cfgs = [
    ...
    elif args.dataset == "flir_voc":
        print("loading our dataset...........")
        args.s_imdb_name = "voc_thermal_2007_trainval+voc_thermal_2012_trainval"
        args.t_imdb_name = "flir_tr_2007_trainval"
        args.s_imdbtest_name = "flir_tr_2007_test"
        args.t_imdbtest_name = "flir_tr_2007_test"
        args.set_cfgs = [
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            channel_label_s = Variable(torch.ones(channel_feat_s.size(0),channel_feat_s.size(1)).float().cuda())
            channel_label_s[0,:]=0
            channel_label_s[1,:]=1
            channel_label_s[2,:]=2
            channel_label_s[3,:]=3

            loss_channel_s=F.mse_loss(channel_feat_s,channel_label_s)
            ...
            out_d_pixel, out_d_t, target_ins_da,target_ins_da_pos_neg,channel_feat_t= fasterRCNN(
                im_data,
                im_info,
                select,
                gt_boxes,
            ...
            channel_label_t = Variable(torch.ones(channel_feat_t.size(0),channel_feat_t.size(1)).float().cuda())
            channel_label_t[0,:]=0
            channel_label_t[1,:]=1
            channel_label_t[2,:]=2
            channel_label_t[3,:]=3

            loss_channel_t=F.mse_loss(channel_feat_t,channel_label_t)
            ...
            loss += (source_ins_da_pos_neg + target_ins_da_pos_neg) *0.01
            loss+=(loss_channel_s+loss_channel_t)*0.05
            ...
            if step % args.disp_interval == 0:
                ...
                source_ins_da_pos_neg = source_ins_da_pos_neg.item() *0.01
                target_ins_da_pos_neg = target_ins_da_pos_neg.item()*0.01
                loss_channel_t=loss_channel_t.item()*0.05
                loss_channel_s=loss_channel_s.item()*0.05
                ...
                print(
                    "\t\t\t rpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f dloss s: %.4f dloss t: %.4f dloss s pixel: %.4f dloss t pixel: %.4f eta: %.4f, source ins loss: %.4f, target ins loss: %.4f"
                    % (
                        # loss_category_cls,
                        loss_rpn_cls,
                        loss_rpn_box,
                ...
                print(
                    "\t\t\t  loss_channel_s: %.4f, loss_channel_t: %.4f,s_ins_pos_neg: %.4f, t_ins_pos_neg : %.4f" % (loss_channel_s, loss_channel_t,source_ins_da_pos_neg,target_ins_da_pos_neg)
                )
                loss_temp = 0
                start = time.time()
            if (step % 1000 == 0 and step!=0 ) and epoch>=6:
        ...
        if (epoch % args.checkpoint_interval == 0 or epoch == args.max_epochs) and epoch>=6:
            save_name = os.path.join(
